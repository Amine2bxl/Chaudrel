...
export default function ServiceList({ services = SERVICES, tone = 'dark', className }) {
  ...
  return (
    <ul className={cn('border-t', light ? 'border-paper/15' : 'border-ink/12', className)}>
      {services.map((s, i) => (
        <Reveal
          as="li"
          key={s.slug}
          delay={i * 60}
          className={cn('border-b', light ? 'border-paper/15' : 'border-ink/12')}
        >
          <Link
            to={`/services/${s.slug}`}
          ...
          >
            <span className={cn('t-num text-lg', light ? 'text-paper/65' : 'text-ink/65')}>
            ...
            <h3
              className={cn(
                ...
                light ? 'text-paper group-hover:text-signal' : 'text-ink group-hover:text-signal'
              )}
            >
              {s.title}
            </h3>
