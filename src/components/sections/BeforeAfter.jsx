...
export default function BeforeAfter({ before, after, label, ratio = 'aspect-[4/3]', className, tone = 'dark' }) {
  ...
  return (
    <figure className={cn('group', className)}>
      <div className={cn('relative overflow-hidden rounded-lg bg-raised', ratio)}>
        <img
          key={src}
          ...
          {[
            ...
            <button
              ...
              className={cn(
                ...
                b.active ? 'bg-cream text-ground shadow-soft' : 'text-cream/75 hover:text-cream'
              )}
            >
              {b.text}
            </button>
      ...
      <figcaption className={cn('t-small mt-4 border-t pt-3', light ? 'border-cream/15 text-cream/65' : 'border-cream/10 text-cream/65')}>
        {label}
      </figcaption>
    </figure>
  );
