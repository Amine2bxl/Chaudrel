 ...
 * vers le bas à droite, sans nœud. Chaque repère porte son numéro ET son
 * titre - l'étape se lit d'un coup d'œil, sans étiquette à distance. La page
 * (accueil) ajoute, elle, la seule sortie : « Découvrir notre méthode ».
 */
    ...
    <div ref={ref} className={className}>
      ...
      <div className="relative hidden lg:block">
        <div className="relative" style={{ height: H }}>
          ...
          {/* Les repères : numéro dans la bulle, titre juste dessous. */}
          {POINTS.map((p, i) => {
            const step = steps[i];
            ...
            return (
              <div
                key={step.n}
                className={cn(
                  'absolute flex flex-col items-center transition-[opacity,transform] duration-[800ms] ease-soft',
                  drawn ? 'opacity-100' : 'opacity-0'
                )}
                style={{
                  left: `${(p.x / W) * 100}%`,
                  top: `calc(${(p.y / H) * 100}% - 30px)`,
                  transform: drawn ? 'translateX(-50%)' : 'translateX(-50%) translateY(16px) scale(0.9)',
                  transitionDelay: `${240 + i * 160}ms`,
                }}
              >
                <span
                  className={cn(
                    'grid place-items-center rounded-full transition-colors duration-slow',
                    done
                      ? light
                ...
                <h3
                  className={cn(
                    't-h3 mt-3 text-center text-balance',
          ...
          return (
            ...
            >
              <span
                ...
                className={cn('relative z-10 grid h-14 w-14 flex-none place-items-center rounded-full', nodeClass(last), last ? 'ring-green/15' : light ? 'ring-gold-light/15' : 'ring-gold/15')}
